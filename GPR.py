...
class GPR:
    ...
    def load_pretrained(self, path):
        ...
        with open(os.path.join(path, 'config_new.pkl'), 'rb') as f:
        ...
        state_dict = torch.load(os.path.join(path, 'model_best.pt'), map_location = torch.device('cpu'))
    ...
    def save_model(self, path):
        ...
        with open(os.path.join(path, 'config_new.pkl'), 'wb') as f:
            pickle.dump(self.config, f)
       
        torch.save(self.best_model.state_dict(), os.path.join(path, 'model_best.pt'))
    ...
    def predict(self, pert_list):
        ...
        for pert in pert_list:
            ...
            cg,Xs = create_cell_graph_dataset_for_prediction(pert, self.ctrl_adata,
    ...
    def GI_predict(self, combo, GI_genes_file='./genes_with_hi_mean.npy'):
        ...
        Returns
        -------
        GI scores for the given combinatorial perturbation 
        """
    ...
    def plot_perturbation(self, query, save_file = None):
        ...
        pred_ = self.predict([query_])['_'.join(query_)]
        pred = np.mean(pred_,axis = 0)[de_idx]
        ...
        for i in range(pred.shape[0]):
            _ = plt.scatter(i+1, pred[i], color='#b8a9d0')
        ...
        return pred_ , truth, genes , de_idx
