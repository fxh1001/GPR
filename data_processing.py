...
class Data_p:
    ...
    def set_pert_genes(self):
        ...
        # self.pert_names = np.unique(list(gene_name))
    ...
    def filter_pert_in_go(self,condition, pert_names):
        """
        ...
        if condition == 'ctrl':
            ...
            if num_ctrl + num_in_perts == 2:
                ...
                return False
    def get_pertname(self):
        pert_name_list = []
        for p in self.adata.obs['condition'].unique():
            for pert_name in p.split('+'):
                if pert_name != "ctrl" and pert_name in self.adata.var.gene_name.values:
                    pert_name_list.append(p)
        return pert_name_list
    ...
    def load(self, data_name = None, data_path = None):
        ...
        self.pert_name_list = self.get_pertname()
        perts = np.unique(self.pert_name_list)
        perts_ = self.get_genes_from_perts(perts)
        filter_gene = self.adata.obs[self.adata.obs.condition.apply(
                              lambda x: self.filter_pert_in_go(x, perts_))]
        self.adata = self.adata[filter_gene.index.values, :]
        
        pyg_path = os.path.join(data_path, 'data_new')
